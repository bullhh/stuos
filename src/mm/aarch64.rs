...
impl A64PTE {
    pub fn new_page(paddr: usize, flags: MappingFlags, is_huge: bool) -> Self {
        let mut attr = DescriptorAttr::from(flags) | DescriptorAttr::AF;
        if !is_huge {
            attr |= DescriptorAttr::NON_BLOCK;
        }
        Self(attr.bits() | (paddr as u64 & Self::PHYS_ADDR_MASK))
    }
    ...
    pub fn paddr(&self) -> usize {
        usize::from((self.0 & Self::PHYS_ADDR_MASK) as usize)
    }
    pub fn flags(&self) -> MappingFlags {
        DescriptorAttr::from_bits_truncate(self.0).into()
    }
    pub fn set_paddr(&mut self, paddr: usize) {
        self.0 = (self.0 & !Self::PHYS_ADDR_MASK) | (paddr as u64 & Self::PHYS_ADDR_MASK)
    }
    pub fn set_flags(&mut self, flags: MappingFlags, is_huge: bool) {
        let mut attr = DescriptorAttr::from(flags) | DescriptorAttr::AF;
        if !is_huge {
            attr |= DescriptorAttr::NON_BLOCK;
        }
